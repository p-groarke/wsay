...
NK_API struct nk_context* nk_glfw3_init(
		GLFWwindow* win, enum nk_glfw_init_state);
NK_API void nk_glfw3_shutdown(void);
NK_API void nk_glfw3_font_stash_begin(struct nk_font_atlas** atlas);
NK_API void nk_glfw3_font_stash_end(void);
NK_API void nk_glfw3_new_frame(void);
NK_API void nk_glfw3_render(
		enum nk_anti_aliasing, int max_vertex_buffer, int max_element_buffer);
...
NK_API void nk_gflw3_scroll_callback(
...
NK_INTERN void nk_glfw3_clipboard_paste(
		nk_handle usr, struct nk_text_edit* edit) {
...
NK_API struct nk_context* nk_glfw3_init(
		GLFWwindow* win, enum nk_glfw_init_state init_state) {
	glfw.win = win;
...
NK_API void nk_glfw3_new_frame(void) {
	...
	nk_input_key(
			ctx, NK_KEY_DEL, glfwGetKey(win, GLFW_KEY_DELETE) == GLFW_PRESS);
	nk_input_key(
	...
	nk_input_key(
			ctx, NK_KEY_DOWN, glfwGetKey(win, GLFW_KEY_DOWN) == GLFW_PRESS);
	...
	if (glfwGetKey(win, GLFW_KEY_LEFT_CONTROL) == GLFW_PRESS
		...
		nk_input_key(
				ctx, NK_KEY_COPY, glfwGetKey(win, GLFW_KEY_C) == GLFW_PRESS);
		nk_input_key(
				ctx, NK_KEY_PASTE, glfwGetKey(win, GLFW_KEY_V) == GLFW_PRESS);
		nk_input_key(
				ctx, NK_KEY_CUT, glfwGetKey(win, GLFW_KEY_X) == GLFW_PRESS);
